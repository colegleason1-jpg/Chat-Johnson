...
|---|---|---|
| `app.py` | Streamlit studio: sidebar (keys, Heavy Mode, paid slot, artifacts, memory, roadmap stubs), four workspace tabs, per-workspace thread bar, Task Finder, preview canvas | credential_fingerprint, get_quota_ledger, get_task_request_lock, build_marker, configured_provider_names, active_mode, session_paid_slot, provider_status_rows, build_prompt_messages, model_refine_digest, health_sweep, display_language, infer_artifact_path, render_output_with_artifacts |
...
| `orchestrator/providers.py` | legacy OpenAI-compatible + Gemini client with retries, model recovery, key redaction | ProviderError, chat |
...
| `orchestrator/vault.py` | SQLite: threads per workspace, messages, window/texturize/archive, summaries, artifacts, health sweep, vision digest, migration | database_path, initialize_database, active_thread, thread_by_id, list_threads, create_thread, switch_thread, rename_thread, set_thread_status, clear_thread, estimate_tokens, redact_secrets, append_message, extractive_summary |
| `orchestrator/missions.py` | deterministic mission classification + workstream templates for Task Finder | classify_mission, task_plan |
