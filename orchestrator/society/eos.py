"""Traction/EOS mechanics: the weekly scorecard, the Level 10 agenda and minutes, timeline drift."""
...
DAY = 86_400.0
_MINUTE_LINE = re.compile(r"^\s*(HEADLINE|ISSUE|TODO|QUESTION)\s*:\s*(.+?)\s*$", re.I)
...
def _week_start(timestamp: float = 0.0) -> float:
...
def scorecard_review(scope: str, company_id: int, cycle_id: int) -> List[Dict[str, Any]]:
    """Compute this week's KPI actuals per filled seat from the vault; returns the misses."""
    week = iso_week()
    since = _week_start()
    ...
    cycles_this_week = sum(1 for c in store.cycles_for(scope, company_id, limit=50) if float(c["started_at"]) >= since)
    for seat in store.seats_for(scope, company_id, status="filled"):
        ...
        for kpi, target in kpis.items():
            if kpi == "deliverables":
                ...
                actual = float(cycles_this_week + 1)  # this cycle counts
            else:
                actual = 0.0
...
def parse_minutes(text: str) -> Dict[str, List[str]]:
    out: Dict[str, List[str]] = {"headline": [], "issue": [], "todo": [], "question": []}
