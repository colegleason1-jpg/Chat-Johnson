...
- The canvas says "N problem(s) found" before you run anything: that is the static linter, and each problem is a
  reason the page will not work (a handler calling an undefined function, an element the script cannot find, a
  blocked external resource, an empty body, fewer items than you asked for). Ask for those specific fixes.
...
- Run the page shows errors but nothing gets fixed: automatic fixes need Heavy Mode on and the
  ...
  (one `sandbox_fix` setting per scope, newest 8 pages), so a reload does not restart them; a changed
  page starts fresh. A page that only depends on an external script or stylesheet is reported as
  "blocked" and fixed the same way; blocked images or fonts are noted, not fixed. The sealed frame
  has no network, storage, dialogs, downloads, clipboard or form submit: each is reported as blocked,
  and the PREVIEW RULES sent with every Run-mode send tell the model to avoid them (inline everything,
  simulate data, buttons plus Enter instead of forms, pointer-event drag, console.error on caught
  failures so the report sees them). Run mode executes what Render or the chat committed, never an
  unrendered draft in the box.
  The frame is a Streamlit component served from `frontend/sandbox_preview` (the vendored Three.js
  is 0.9 MB, regenerated by `scripts/build_three_datauri.py`), so a deploy must ship that directory.
  "No ready signal" means the page never finished loading (a blocking loop); "navigated" means it
  tried to load another address, which the frame refuses.
